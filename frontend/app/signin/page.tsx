  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setAlert({ message: 'Signin successfully!!', type: 'success' });
      ...
      if (axios.isAxiosError(error) && error.response) {
        ...
        if (status === 400 && data?.error === 'Invalid credentials') {
          setAlert({ message: 'توجد مشكلة في بيانات تسجيل الدخول. يرجى التحقق من اسم المستخدم/البريد الإلكتروني وكلمة المرور.', type: 'error' });
        } else if (status === 500) {
          setAlert({ message: 'حدث خطأ في الخادم. يرجى المحاولة مرة أخرى لاحقًا.', type: 'error' });
        } else {
          setAlert({ message: data?.message || 'حدث خطأ غير متوقع أثناء تسجيل الدخول.', type: 'error' });
        }
      } else {
      ...
      <FadeIn delay={200}>
        <Card className="w-full max-w-md p-8 transition-transform duration-500 border border-[#00A6C0] rounded-3xl [box-shadow:0_0_30px_rgba(0,166,192,0.6)]">
          ...
          <p className="text-[#D8D7CE] text-center mb-8 opacity-80 leading-snug">
            سجل الدخول للمتابعة إلى جلستك الآمنة والوصول إلى لوحة التحكم المخصصة لك.
          </p>
          <form className="space-y-6" onSubmit={handleSignIn}>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                <input type="checkbox" id="remember_me" className="h-5 w-5 text-[#00A6C0] rounded border-gray-600 focus:ring-[#00A6C0] transition duration-300" />
                <label htmlFor="remember_me" className="mr-2 block text-[#D8D7CE] text-sm font-medium">تذكرني</label>
              </div>
              <Link href="/forgot-password" className="text-sm font-medium text-[#00A6C0] hover:underline transition duration-300">
                هل نسيت كلمة المرور؟
              </Link>
            </div>
            <Button
          ...
          <div className="mt-8 text-center text-[#D8D7CE] text-md">
            ليس لديك حساب؟{" "}
            <Link href="/signup" className="text-[#00A6C0] hover:underline font-semibold transition duration-300">
              سجل الآن
            </Link>
          </div>
